...
// 3. Busca todos os times aprovados nas inscrições (com o logo)
$stmt_times = $pdo->query("\n    SELECT i.campeonato_id, t.id, t.nome, t.caminho_logo \n    FROM inscricoes i \n    JOIN times t ON i.time_id = t.id \n    WHERE i.status = 'aprovada'\n");
...
function inicializaTime(&$array, $cid, $tid, $nome, $logo = null)
{
    if (!isset($array[$cid][$tid])) {
        ...
        // Se já existe, aproveita para preencher o logo caso ainda esteja vazio
        if ($logo !== null && empty($array[$cid][$tid]['logo'])) {
            $array[$cid][$tid]['logo'] = $logo;
        }
    }
}
...
foreach ($inscricoes as $insc) {
    inicializaTime($classificacao, $insc['campeonato_id'], $insc['id'], $insc['nome'], $insc['caminho_logo']);
}

// Passo B: Garante que os times que já têm jogos agendados também entrem na tabela
foreach ($todas_partidas as $p) {
    inicializaTime($classificacao, $p['campeonato_id'], $p['time_a_id'], $p['time_a'], $p['logo_a'] ?? null);
    inicializaTime($classificacao, $p['campeonato_id'], $p['time_b_id'], $p['time_b'], $p['logo_b'] ?? null);
}
